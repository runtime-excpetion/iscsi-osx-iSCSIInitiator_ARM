...
/* Version history:
 ...
 1.4  28 Jul 2026  Added ARM64 software table fallback for Apple Silicon builds.
 */

#include <stdint.h>
#include <stddef.h>

/* CRC32C function prototypes — portable across kext (IOKit) and userspace. */
...
/* Software lookup table, populated at init time. */
...
void crc32c_init()
{
    ...
    /* Build software byte-by-byte CRC32C lookup table. */
    for (uint32_t n = 0; n < 256; n++) {
        uint32_t val = n;
        for (int k = 0; k < 8; k++)
            val = (val >> 1) ^ (POLY & -(val & 1));
        crc32c_table[n] = val;
    }
}
...
uint32_t crc32c(uint32_t crc,const void * buf,size_t len)
{
    ...
    // Intel x86_64 — use hardware CRC32 instructions
...
#else
    // Portable software CRC32C — byte-by-byte with lookup table.
    // Used on ARM64 (Apple Silicon) and any other architecture.
    const unsigned char *bytes = (const unsigned char *)buf;
    uint32_t crc_val = crc ^ 0xffffffff;
    for (size_t i = 0; i < len; i++)
        crc_val = crc32c_table[(crc_val ^ bytes[i]) & 0xff] ^ (crc_val >> 8);
